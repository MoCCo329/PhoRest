...
class MainWindow(QMainWindow, Main_Ui.Ui_MainWindow):
    ...
    def keyReleaseEvent(self, e):
        if self.stack.currentIndex() == 4:
            if e.key() == Qt.Key_Enter and self.Frame_Id.text() != "":
                self.Press_Applying()
        else:
            pass
    ...
    def RecommandPose(self):
        ...
        self.js = json.loads(res.text)
        if len(self.js) == 0:
            return

        if self.ChooseBtnNum == 1:
            self.Thisidx = self.idx_1
        elif self.ChooseBtnNum == 2:
            self.Thisidx = self.idx_2
        elif self.ChooseBtnNum == 3:
            self.Thisidx = self.idx_3
        elif self.ChooseBtnNum == 4:
            self.Thisidx = self.idx_4
        elif self.ChooseBtnNum == 5:
            self.Thisidx = self.idx_5
        elif self.ChooseBtnNum == 6:
            self.Thisidx = self.idx_6


        url = self.js[self.Thisidx].get("url")
        Pose_Frameid = str(self.js[self.Thisidx].get("frameId"))

        self.Pose_applingFrameId.setText(Pose_Frameid)
    ...
    def Press_Applying(self):
        ...
        if(self.frame_id == ""):
            ...
            if(url == "fail"):
                ...
                self.Warning_label.hide()
                os.system("curl " + url + " > ./Frame/Frame_{}.png".format(self.frame_id))
                time.sleep(0.5)
                ...
                self.FrameNum = int(self.frame_id)
